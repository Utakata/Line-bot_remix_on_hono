...
# def generate_response_with_cache(client, model, prompt, max_tokens, temperature, system_content):
def generate_response_with_cache(prompt, max_tokens, temperature, system_content):
    """
    プロンプトキャッシュを使用してAnthropic APIから応答を生成する関数。

    引数:
        prompt (str): 応答を生成するためのプロンプト
        max_tokens (int): 生成する最大トークン数
        temperature (float): 生成の温度パラメータ
        system_content (str): システムメッセージの内容
